        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div id="wizard_container_staff">
                    ...
                    <form action="<?php echo base_url();?>admin/quote/create" method="POST" id="wrapped" name="wizard_container">
                        <div id="middle-wizard">
                                ...
                                <div class="row">
                                    ...
                                    <select class="itemName form-control select2" required=""  name="practice_id" onchange="selectServices(0)" >
										  <option value="">Seleccionar</option>
										    ...
										    <?php 
										        $inventorys = $this->db->query('SELECT * FROM inventory where status = 1 and clinic_id ='.$clinic_id)->result_array();
                                                foreach($inventorys as $inventory):
                                                $products = $this->db->query('SELECT ip.inventory_id,ip.product_id, p.code, p.name FROM `inventory_product` ip INNER JOIN product p ON ip.product_id = p.product_id WHERE ip.status = 1 and ip.inventory_id = '.$inventory['inventory_id'])->result_array();
                                                if(count($products)>0):
                                            ?>
                                                 <optgroup label="<?php echo $inventory['name']; ?>">
                                                     <?php 
                            ...
                            <div class=" submit step">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-3" style="display:none;" id="insurance">
                                        <div class="form-group">
                                            <label>Seguro</label>
                                            <select class="itemName form-control select2" style="width:100%" name="insurance_id">
                                                   <option value="">Seleccionar</option>
                                                <?php
                                                $this->db->where('status', '1');
                                                $this->db->where('clinic_id', $this->session->userdata('current_clinic'));
                                                ...
                                                foreach ($insurances as $insurance):
                                                    ?>
                                                    <option value="<?php echo $insurance['insurance_id']; ?>"><?php echo $insurance['name']; ?></option>
                                                <?php endforeach; ?>
...
<script src="<?php echo base_url();?>public/assets/appointments/js/appointment_form.js?v=1.2"></script>