    ...
    <?php
     ...
     $bam = $this->crud_model->getAccountByBankName("BAM");
        ...
        foreach($info as $details):
         ...
         if($details['status']==6):?>
    <form action="<?php echo base_url();?>admin/appointments/pay_1/<?php echo $_id; ?>" method="POST">
        <?php endif;?>
        
        <?php if($details['status']==10):?>
    <form action="<?php echo base_url();?>admin/appointments/pay_2/<?php echo $_id; ?>" method="POST">
        <?php endif;?>
                        ...
                        <div class="dropdown" style="float:right">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="width:auto; text-align:left;overflow-y:hidden">
                                <?php if($details['status'] == 1):?>
                                    <a class="dropdown-item" target="_blank" href="<?php echo base_url();?>admin/print_receipt/<?php echo $details['invoice_1'];?>/<?php echo $details['patient_id'];?>">Imprimir</a>
                        ...
                        <div class="col-lg-12 col-md-12 col-ms-12 mb-2 row">
                            ...
                            <div class="col-lg-12 col-md-12 col-ms-12 mb-2">
                                <div class="form-group">
                                    ...
                                    <select name="establecimiento_id" id="institution_id" class="form-select form-control" required onchange="getDataInstitution()">
            ...
            <div class="order-box">
                <div class="order-details-box">
                    ...
                    <div class="dropdown" style="float:right">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="width:auto; text-align:left;overflow-y:hidden">
                            <a class="dropdown-item" target="_blank" href="<?php echo base_url();?>admin/patient_profile/<?php echo base64_encode($details['patient_id']);?>/">Perfil del paciente</a>
                            <a class="dropdown-item" href="<?php echo base_url();?>admin/pay_appointment/email/<?php echo $_id;?>/<?php echo $details['patient_id']?>">Enviar recibo por correo</a>
                            <?php if($prescription_id > 0):?>
                            <a class="dropdown-item" target="_blank" href="<?php echo base_url();?>admin/print_prescription_details/<?php echo $_id;?>">Imprimir receta</a>
                            <?php endif;?>
...
function send_email(app_id) {
    Swal.fire({
        ...
        if (result.value) {
            location.href = "<?php echo base_url();?>admin/pay_appointment/email/" + app_id;
        }
    })
}
</script>
...
function getLaboratories() {
    $.ajax({
        url: '<?php echo base_url();?>admin/labStatus/' + <?php echo base64_decode($id_);?>,
        success: function(response) {
            if (response != "")
...
function labStatus(exam) {
    $.ajax({
        url: '<?php echo base_url();?>admin/samples/pay_exam/' + exam,
        success: function(response) 
        {
            getLaboratories()
        },
...
function rayStatus(exam) {
    $.ajax({
        url: '<?php echo base_url();?>admin/rayx/pay_exam/' + exam,
        success: function(response) 
        {
            getRayos()
        }
...
function getRayos() {
    $.ajax({
        url: '<?php echo base_url();?>admin/rayx/get_service/' + <?php echo base64_decode($id_);?>,
        success: function(response) {
