            ...
            <div class="card-box">
                <div class="card-b">
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            <form class="repeater" action="<?php echo base_url();?>staff/opening_departure/create/" method="POST" enctype="multipart/form-data" id="frmR">
                                <div class="row">
...
<script type="text/javascript">
    ...
    function verifyPeriod() {
        ...
        if ($.isNumeric(period) && period.length == 4) {
            $.ajax({
                type: "POST",
                url: "<?php echo base_url();?>staff/verifyPeriodOpen/",
                data: {
                    period: period,
                },
                dataType: "json",
    ...
    function chargeDeparture() {
        ...
        if ($.isNumeric(period) && period.length == 4) {
            if (!ldD) {
                $.ajax({
                    method: "POST",
                    url: "<?php echo base_url();?>staff/chargeOpenDeparture/",
                    data: {
                        year: period,
                    },
                    beforeSend: function () {
    ...
    function verifyForm() {
        ...
        if (valForm && chD) $("#frmR").attr("action", "<?php echo base_url();?>staff/opening_departure/create/");
