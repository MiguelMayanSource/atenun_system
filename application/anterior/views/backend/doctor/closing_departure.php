            ...
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            <form class="repeater" action="<?php echo base_url();?>doctor/closing_departure/create/" method="POST" enctype="multipart/form-data" id="frmC">
                                <div class="row">
...
<script type="text/javascript">
    ...
    function verifyPeriod() {
        ...
        if (category != '' && $.isNumeric(period) && period.length == 4 && ((type == 'month' && month != '') || type == 'year')) {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url();?>doctor/verifyPeriodClose/",
                data: {
                    category: category,
                    type: type,
                    period: period,
    ...
    function chargeDeparture() {
        ...
        if (category != '' && $.isNumeric(period) && period.length == 4) {
            if (!ldD) {
                $.ajax({
                    method: "POST",
                    url: "<?php echo base_url();?>doctor/chargeCloseDeparture/",
                    data: {
                        category: category,
                        type: type,
                        year: period,
    ...
    function verifyForm() {
        var mensaje = '';
        if (valForm && chD && valMonth) $("#frmC").attr("action", "<?php echo base_url();?>doctor/closing_departure/create/");
