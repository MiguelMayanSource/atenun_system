...
<div id="main-content">
    ...
    <form action="<?php echo base_url();?>admin/purchases/create_solicitud" method="POST">
        <div class="row">
            <div class="col-md-12">
                <div class="order-box">
                    ...
                    <div class="order-items-table">
                        ...
                        <div class="social-l col-lg-12 col m-b-30">
                            <div class="order-controls row">
                                ...
                                <div class="col-md-4 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control form-control-sm select2"  name="provider_id" id="provider_id" required="" onchange="selectProvider(this.value)">
                                            <option value="">Seleccionar</option>
                                            <?php 
                                                $query = $this->db->get_where('staff', array('category' => '3','status'=>1))->result_array();
                                                foreach($query as $pat):
                                            ?>
                                            <option value="<?php echo $pat['staff_id'];?>"><?php echo $pat['first_name'];?></option>
                                            <?php endforeach;?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div> <button type="button" class="btn btn-info btn-sm btn-products" style="margin-bottom:10px;" onclick="addProduct()">Agregar Producto</button></div>

                            <table class="table " id="products"></table>
                            <div><button type="button" class="btn btn-info btn-sm btn-services" style="margin-bottom:10px; display:none" onclick="addService()">Agregar servicio</button></div>
                        ...
                        <div class="order-foot">
                            <div class="row">
                                <div class="col-md-12 mb-4">
                                    <h5>Nota: </h5>
                                    <div class="form-group">
                                        <textarea class="form-control" name="description" placeholder="Escribe una nota a esta compra..." rows="7"></textarea>
                                    </div>
                                    <button class="btn btn-success pull-right" type="submit">Confirmar compra</button>
                                </div>
...
function getServicesProvider(provider_id, variant_id) {
    $.ajax({
        url: '<?php echo base_url();?>admin/get_services_provider/' + provider_id,
...
function getProductsProvider(provider_id, variant_id) {
    $.ajax({
        url: '<?php echo base_url();?>admin/get_products_provider/' + provider_id,
        success: function(response) {
            console.log(response);
            if (response != "")
...
function getPres(product_id, variant_id) {
    $.ajax({
        ...
        success: function(response) {
            console.log(response);
            if (response != "")
                jQuery('#unity_' + variant_id).html(` <select name = "unity[]" class = "form-control"  >${response} < /select>`);
            else
        ...
        <td >
            <div class="form-group">
                <div class="input-group">
                    <input type="number" step="any" class="form-control" name="amount[]"  onchange="getSbtotal('${$id}')" onKeyUp="getSbtotal('${$id}')" id="amount_${$id}">
                    <div class="input-group-append">
                        <select name="unity[]" class="form-control" id="unity_${$id}" >
                        </select>
                    </div>
                </div>
            </div>
        ...
        <td >
            <div class="form-group">
                <div class="input-group">
                    ...
                    <input  type="number" step="any" class="form-control" name="sbtotal[]"  id="sb_${$id}" onchange="getSbtotal('${$id}')" onKeyUp="getSbtotal('${$id}')">
                </div>
            </div>
        </td>
        <td class="col-sm-1">
...
function addService() {
    ...
    <tr>
        <td >
            <select class="form-control mb3 select2"  name="service_id[]" id="ser_${$id}" >
                <option value="">Seleccionar</option>
            </select>
        </td>
        <td >
            <div class="form-group">
                <div class="input-group">
                    ...
                    <input type="number" step="any" class="form-control" name="service_subtotal[]"  id="ser_${$id}" onchange="getSbtotal('${$id}')" onKeyUp="getSbtotal('${$id}')">
                </div>
            </div>
        </td>
        <td>
